...
Color indicator_color( bool b )
{
    return b == OFF ? Color::GrayLight : Color::Yellow;
}

Color door_color( bool b )
{
    return b == OFF ? Color::Green : Color::Red;
}

void display_to_terminal()
{
    auto document =  //
        vbox(
        {
            ...
            hbox(
            {
                filler(),
                text("Left indicator "),
                bgcolor( indicator_color( turn_status[0] ), 
                         text( turn_status[0] ? " On  " : " Off " ) ),
                filler(),
                
                bgcolor( indicator_color( turn_status[1] ), 
                         text( turn_status[1] ? " On  " : " Off " ) ),
                text(" Right indicator"),
                filler()
            } ),
            filler(),
            hbox( { filler(), text("Speed"), filler() } ),
            gaugeRight( current_speed * 0.01f ),
            hbox(
            {
                filler(),
                text( std::to_string( int(current_speed) ) + " mph" ),
                filler()
            } ),
            filler(),
            hbox( { filler(), text("Doors"), filler() } ),
            hbox(
            {
                filler(),
                text("Front left "),
                bgcolor( door_color( door_status[0] ), 
                         text( door_status[0] ? " Unlocked " : "  Locked  " ) ),
                filler(),
                bgcolor( door_color( door_status[1] ), 
                         text( door_status[1] ? " Unlocked " : "  Locked  " ) ),
                text(" Front right"),
                filler()
            } ),
            hbox(
...
//  SDL_Event event;
  ...
  auto prevDraw = std::chrono::steady_clock::now();
  ...
  while(running) {
    ...
    if( now - prevDraw >= std::chrono::milliseconds( 40 ) )
    {

      /*std::cout << "door: ";
      for( int i=0; i<4; ++i )
        std::cout << door_status[i] << " ";
      std::cout << std::endl;

      std::cout << "turn: ";
      for( int i=0; i<2; ++i )
        std::cout << turn_status[i] << " ";
      std::cout << std::endl;
