...
int currentTime;
auto lastAccel = std::chrono::steady_clock::now();
auto lastTurnSignal = lastAccel;
...
void checkAccel() {
	...
	if( now - lastAccel > std::chrono::milliseconds(10) ) 
	{
		if(!throttle) {
			current_speed -= rate;
...
void checkTurn() 
{
	...
	if( now - lastTurnSignal > std::chrono::milliseconds(500) ) 
	{
		if(turning < 0) {
...
void turn(int value) {
	...
	turning = std::min<int>( 1, std::max<int>( -1, value ) );
}
...
*/
 /* while(running) {
    ...
    // == options ======
    std::vector<std::string> turnOptions { "Left", "Straight", "Right" };
    int turnSelect = 1;
    bool accelerator = false;
    int slider;

    // a collection of random options to show ui interaction
    auto options = Container::Vertical(
    {
        Checkbox( "Accelerator", &throttle )
    } );
    options = Wrap("Options", options);

    auto indicator = Container::Horizontal(
    {
        Radiobox( &turnOptions, &turnSelect )
    } );
    indicator = Wrap( "Indicator", indicator );

    auto lock = Container::Horizontal(
    {
        Button( "All", []{ send_lock(CAN_DOOR1_LOCK | 
									 CAN_DOOR2_LOCK | 
									 CAN_DOOR3_LOCK | 
									 CAN_DOOR4_LOCK); } ),
        Button( "FL", []{ send_lock(CAN_DOOR1_LOCK); } ),
		Button( "FR", []{ send_lock(CAN_DOOR2_LOCK); } ),
		Button( "RL", []{ send_lock(CAN_DOOR3_LOCK); } ),
		Button( "RR", []{ send_lock(CAN_DOOR4_LOCK); } )
    } );
    lock = Wrap( "Lock", lock );

    auto unlock = Container::Horizontal(
    {
        Button( "All", []{ send_unlock(CAN_DOOR1_LOCK | 
									   CAN_DOOR2_LOCK | 
									   CAN_DOOR3_LOCK | 
									   CAN_DOOR4_LOCK); } ),
        Button( "FL", []{ send_unlock(CAN_DOOR1_LOCK); } ),
		Button( "FR", []{ send_unlock(CAN_DOOR2_LOCK); } ),
		Button( "RL", []{ send_unlock(CAN_DOOR3_LOCK); } ),
		Button( "RR", []{ send_unlock(CAN_DOOR4_LOCK); } )
    } );
	...
	std::thread processThread = std::thread( [&]
	{
		while( running )
		{
			turning = turnSelect -1;
			checkAccel();
			checkTurn();			
